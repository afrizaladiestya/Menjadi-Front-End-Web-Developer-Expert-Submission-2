...
<article tabindex="0" class="resto-item">
  <a href="/#/detail/${restaurant.id}">
    ...
    <img class="resto-item_image " src="${CONFIG.BASE_IMAGE_URL + restaurant.pictureId}" alt="Gambar ${restaurant.name}" tabindex="0"/>
    <p tabindex="0" class="resto-item_name" alt="nama restoran">${restaurant.name}</p>
      <div class="card-location-rating">     
          <div class="card-city">
              <i class="fas fa-map-marker-alt"></i>
              <span class="card-city-text">${restaurant.city} </span>
          </div>
          <div class="rating">
...
<div class="detail">
  <div tabindex="0" class="container-info">
    ...
    <img class="resto-item_image-container " src="${CONFIG.BASE_IMAGE_URL + restaurant.pictureId}" alt="Gambar ${restaurant.name}" tabindex="0"/>
    </div>
    ...
    .join('')}
      ...
      <h2 tabindex="0" id="resto-detail"><span>List Menu</span></h2>
    ...
    .map(
      (food) => `
              <li><i class="fa fa-circle"></i> ${food.name}</li>`,
    )
    .join('')}
            </ul>
          </div>
    ...
    .map(
      (drink) => `
              <li><i class="fa fa-circle"></i> ${drink.name}</li>`,
    )
    .join('')}
        </div>
      </div>
